\section{Grundlagen}
   ...
   \lstset{language=PHP} 
	...
	\begin{minipage}{\linewidth}
		...
		\captionof{figure}[BasicLogic]{UML Modellierung der Klasse \texttt{CatalogToolsTask}}
		\vspace{1em}
	\end{minipage}
	...
	\begin{minipage}{\linewidth}
		...
		\captionof{figure}[BasicLogic]{UML-Klassendiagramm der \texttt{UpdateCatalogTask}}
		\vspace{1em}
	\end{minipage}
	
	Die Funktion \texttt{checkMercateoProductsStatusChanges(\$sellerId)} fasst jene
	...
	\texttt{testArticleDataValidator()} führt einen Positiv- und einen Negativtest durch, indem jeweils eine 'gültige' bzw. 'ungültige' Instanz von \textit{CoreProducts} an \texttt{articleDataValidator(\$product)} übergeben wird.
	
	Mit \texttt{testCheckImageFormat()} wird geprüft, ob \texttt{checkImageFormat(\$image)} jeweils \texttt{true} zurückliefert wenn der übergebene \textit{URI} auf 'jpeg', 'jpg' oder 'gif' endet. Es wird \texttt{false} erwartet, wenn die Datei eine andere Endung hat.
